...
/**
 * BarChart Component
 * Displays bar chart using Recharts with customizable data and styling
 * 
 * @param {array} data - Chart data array
 * @param {array} dataKeys - Keys to display as bars
 * @param {string} xAxisKey - Key for X-axis
 * @param {string} title - Chart title
 * @param {number} height - Chart height in pixels
 * @param {array} colors - Color array for bars
 ...
 * @param {string} layout - Bar layout ('vertical' or 'horizontal')
 * @param {boolean} stacked - Stack bars on top of each other
 * @param {string} barShape - Bar corner radius ('rounded' or 'square')
 * @param {number} barSize - Custom bar width
 */
const BarChart = ({ 
  data = [], 
  dataKeys = [], 
  ...
  showGrid = true,
  loading = false, // ✅ Added
  onBarClick = null, // ✅ Added
  unit = '', // ✅ Added
  layout = 'vertical', // ✅ Added
  stacked = false, // ✅ Added
  barShape = 'rounded', // ✅ Added
  barSize = null, // ✅ Added
}) => {
  // ✅ Validate data
  ...
  // ✅ Get bar radius based on shape
  const getBarRadius = () => {
    if (barShape === 'square') return [0, 0, 0, 0];
    return [4, 4, 0, 0];
  };
  ...
  return (
    <div className="w-full">
      ...
      <div 
        ...
        role="img" // ✅ Added
        aria-label={title || 'Bar Chart'} // ✅ Added
      >
        <ResponsiveContainer width="100%" height={height}>
          <RechartsBarChart
            data={validData}
            layout={layout === 'horizontal' ? 'vertical' : 'horizontal'} // ✅ Handle layout
            margin={{ top: 5, right: 30, left: 20, bottom: 5 }}
            onClick={onBarClick} // ✅ Added
          >
            ...
            <XAxis 
              ...
              stroke="#6b7280"
              style={{ fontSize: '12px' }}
              type={layout === 'horizontal' ? 'number' : 'category'} // ✅ Added
            />
            ...
            <YAxis 
              ...
              type={layout === 'horizontal' ? 'category' : 'number'} // ✅ Added
            />
            ...
            {validDataKeys.map((key, index) => (
              <Bar
                ...
                radius={getBarRadius()}
                stackId={stacked ? 'stack' : null} // ✅ Added
                barSize={barSize} // ✅ Added
      ...
      {validData.length > 0 && (
        <div className="mt-4 grid grid-cols-4 gap-2 text-xs text-gray-600">
          <div className="bg-gray-50 p-2 rounded">
            <p className="font-semibold">Bars</p>
          ...
          <div className="bg-gray-50 p-2 rounded">
            ...
            <p className="text-gray-800 capitalize">{layout}</p>
          </div>
          <div className="bg-gray-50 p-2 rounded">
            <p className="font-semibold">Stacked</p>
            <p className="text-gray-800">{stacked ? 'Yes' : 'No'}</p>
          </div>
        </div>
      )}
    </div>
